...
class TrackerExcelImporter
  ...
  def call
    ...
    mapped_headers = header_row.map do |header|
      HEADER_MAP[header.to_s.strip]
    end
  ...
  def validate_headers!(header_row)
    normalized_headers = header_row.map(&:to_s)

    missing_headers = HEADER_MAP.keys - normalized_headers

    return unless missing_headers.any?

    raise "Missing headers: #{missing_headers.join(', ')}"
  end
